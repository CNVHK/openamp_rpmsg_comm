...
- 使用实测模型增益 `K = [-152.765302998, -13.260563186, -1.414213562, -5.829463511]`。
...
控制律输出 `u` 为两轮共同水平驱动力，单轮力矩换算为 `tau = u × 0.016125`。5° 初始倾角仿真的最大总水平力为 13.3313 N、最大单轮力矩为 0.2150 N·m。当前第二阶段实机测试限幅为 0.10 N·m；只有在轮子落地、车身可自由俯仰且方向验证完成后，才能逐步提高至 0.22 N·m。

启用时要求绝对俯仰角不超过 5°、俯仰角速度不超过 0.15 rad/s，且任一轮线速度绝对值不超过 0.10 m/s，否则拒绝进入 active。active 状态下平均轮速绝对值超过 1.0 m/s 时立即进入故障并下发零力矩和 idle。
