...
控制律输出 `u` 为两轮共同水平驱动力，单轮力矩换算为 `tau = u × 0.016125`。5° 初始倾角仿真的最大总水平力为 13.3313 N、最大单轮力矩为 0.2150 N·m。当前首轮实机测试限幅为 0.05 N·m；只有在轮子落地、车身可自由俯仰且方向验证完成后，才能逐步提高至 0.22 N·m。

启用时如任一轮线速度绝对值超过 0.10 m/s，控制器拒绝进入 active。active 状态下平均轮速绝对值超过 1.0 m/s 时立即进入故障并下发零力矩和 idle。
